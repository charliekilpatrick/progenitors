    ...
    for type_key in all_keys:
        ...
        for row in type_data:
            try:
                ...
                mask.append(False)
        mask = np.array(mask)
        type_data = type_data[mask]

        mask = []
        for row in type_data:
            try:
                t = float(row['Redshift'])
                mask.append(True)
            except ValueError:
                mask.append(False)
        mask = np.array(mask)
        ...
        mask = (type_data['Redshift'].data.astype(float)<0.015) &\
            (type_data['Redshift'].data.astype(float)>0.)
        type_data = type_data[mask]

        # Define initial target sample
        mask=now-Time(type_data['Discovery Date'].data).mjd<200.0
        if not use_targets:
