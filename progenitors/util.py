"""
Progenitor pipeline utilities: metadata fetching, TNS/YSE/NED/ADS, coordinates, email.
...
# Repo root (parent of progenitors package) for metadata, token, etc.
basedir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
from .settings.util import (
    DM_HIERARCHY,
    EMAIL_FROM_ADDR,
    EMAIL_SMTP_SERVER,
    EMAIL_SMTP_PORT,
...
params = {
    ...
    "SHEET_TEST": os.environ.get("PROGENITORS_SHEET_TEST", ""),
...
dm_hierarchy = DM_HIERARCHY

ads_headers = {"Authorization": "Bearer " + os.environ.get("ADS_AUTHCODE", "")}


def _email_args():
    """Build email config from settings + env (lazy so env can be set after import)."""
    return {
        "from_addr": EMAIL_FROM_ADDR,
        "smtpserver": f"{EMAIL_SMTP_SERVER}:{EMAIL_SMTP_PORT}",
        "gmail_login": os.environ.get("GMAIL_LOGIN", ""),
        "gmail_password": os.environ.get("GMAIL_PASSWORD", ""),
        "to_addr": os.environ.get("MY_EMAIL", ""),
        "subject": EMAIL_SUBJECT,
    }


# For backward compatibility and post_alert use
email_args = _email_args()
email_msg = EMAIL_MSG_TEMPLATE
...
def get_tns_source(row, tns_cert_type):
    ...
    if r.status_code==200:
        reg = r'"https://ui\.adsabs\.harvard\.edu/search/q=.*"'
