"""
Dust and RSG (red supergiant) SED utilities.

Provides dust opacity curves (graphite/silicate, Rout/Rin=2 or 10), RSG model
spectra, and combined star+dust+blackbody spectra for fitting. Data files
are read from data/dust/ (wavelength, opacity, RSG grids).
"""
...
_sed_root = os.path.dirname(os.path.abspath(__file__))
dustdir = os.path.join(_sed_root, 'data', 'dust') + os.sep
...
# Wavelength range (Angstrom) for masked SED operations when not using full grid
WAVE_MASK_MIN_AA = 3500.0
WAVE_MASK_MAX_AA = 1.0e5
...
def rebin(a, newshape):
    """Rebin 1D array `a` to length `newshape` by averaging consecutive bins."""
...
def get_dust(x, p, model='g2'):
    """Dust opacity curve A(lambda). `x` wavelength (Angstrom), `p` tau_V (scalar for g2, else array). `model`: g2, s2, g10, s10."""
...
def get_bb_lum(p, rsg_model='10', dust_model='g2', sptype='all', masked=True):
    """Blackbody luminosity (Lsun) for dust component. p = [tau_V, scale, Teff, Tdust]. sptype: 'all'|'i' (intrinsic)."""
    w = wavelength
    flux = get_rsg(p[1], p[2], model=rsg_model)

    if masked and len(w) != 5157:
        mask = (wavelength > WAVE_MASK_MIN_AA) & (wavelength < WAVE_MASK_MAX_AA)
        kappa = np.loadtxt(dustdir+'dust01_trans.dat')
        w = w[mask]
        flux = flux[mask]
        kappa = kappa[mask]
...
def get_ext_bb(p, rsg_model='10', dust_model='g2', sptype='all', masked=True):
    """Star+dust SED: p = [tau_V, Lsol, Teff, Tdust]. Returns ArraySpectrum; sptype: 'all'|'i'|'s'|'b'."""
    w = wavelength
    flux = get_rsg(p[1], p[2], model=rsg_model)
...
def get_rv(p, rsg_model='10', dust_model='g2'):
    """Stub: computes dust/RSG components but does not return a value. Reserved for future R(V) or related quantity."""
    w = wavelength
    flux = get_rsg(p[1], p[2], model=rsg_model)
...
def get_new_rsg(p, rsg_model='2', dust_model='sil'):
    """Interpolated RSG+dust spectrum from precomputed grid. p = [tau, lum, teff, tdust]. Returns ArraySpectrum in flam."""
    tau = p[0]
    lum = p[1]
    teff = p[2]
    tdust = p[3]

    file = os.path.join(_sed_root, 'data', 'interpolate', f'rsg_{dust_model}_s{rsg_model}_fullgrid.pkl')
