...
from progenitors.sed.analysis import *
...
class sed_plot(object):
    ...
    def load_tracks(self, model_type, params, feh=0.0, clip_early=0.0,
        ...
        if model_type=='mist':
            ...
            for m in params:
                ...
                if filts:
                    for f in filts:
                        for key in cmd_table.keys():
                            if f.upper() in key.upper():
                ...
                if m < 8.0:
                    mask = model_table['star_age'] < 0.999 * np.max(model_table['star_age'])
                    model_table  = model_table[mask]
        ...
        if add_data:
            if limits:
                if self.fit.model_type=='blackbody':
                    ...
                    ax.text(3.8, 5.9, 'Ruled Out', fontsize=5.0*self.figsize)
    ...
    def cmd(self, filt1, filt2, progenitors='progenitors.dat',
        add_progenitors=True,add_yoon=False,add_lbvs=False,add_data=True,
        add_rsg=False,limits=False):

        fig, ax = self.setup_plot()

        color_name = r'$m_{\rm '+filt1.upper()+'}-m_{\rm '+filt2.upper()+'}$'
        absmag_name = r'$M_{\rm '+filt1.upper()+'}$'

        self.setup_axis_titles(ax, color_name, absmag_name)

        # Get range of model track data
        model_masses = [6,8,10,14,17,23,30,40,60]
        tracks = self.load_tracks('mist', model_masses,
                clip_early=3.0e4, clip_tail=4.67, filts=[filt1, filt2])

        def ccolor(r,g,b):
            return((r/255.,g/255.,b/255., 1.0))

        black=ccolor(0,0,0)
        brown=ccolor(165,42,42)
        red=ccolor(255,0,0)
        ...
        pink=ccolor(200,153,159)

        cm = [black, brown, red, orange, goldenrod, green, blue, magenta, pink]

        print(tracks.keys())

        use_filt1 = [f for f in tracks.keys() if filt1.upper() in f.upper()][0]
        use_filt2 = [f for f in tracks.keys() if filt2.upper() in f.upper()][0]

        tracks.add_column(tracks[use_filt1]-tracks[use_filt2], name='color')

        ordrange = np.max(tracks[use_filt1].data)-np.min(tracks[use_filt1].data)
        absrange = np.max(tracks['color'].data)-np.min(tracks['color'].data)

        ylim = [np.max(tracks[use_filt1].data)+0.08*ordrange,
                np.min(tracks[use_filt1].data)-0.025*ordrange]
        xlim = [np.min(tracks['color'].data)-0.18*absrange,
                np.max(tracks['color'].data)+0.12*absrange]
        ...
        for i,mass in enumerate(sorted(np.unique(tracks['mass']))):
            ...
            idx = np.argmax(mass_track['color'].data)
            label_teff = mass_track['color'].data[idx]
            label_logL = mass_track[use_filt1].data[idx]

            ax.plot(mass_track['color'].data, mass_track[use_filt1].data,
                color=cm[i], linewidth=0.4*self.figsize,zorder=1)

            # Get position where track is the hottest to add label
            idx = np.argmin(mass_track['color'].data)
            label_teff = mass_track['color'].data[idx]
            label_logL = mass_track[use_filt1].data[idx]
            textpos = [label_teff+0.0065*self.figsize*(xlim[0]-xlim[1]),
                    label_logL-0.004*self.figsize*(ylim[1]-ylim[0])]
            ...
            ax.text(textpos[0], textpos[1], str(mass)+r'$~M_{\odot}$',
                color=cm[i], zorder=1, horizontalalignment='center',
                verticalalignment='center')


        # Adding data, this is specific to 2021qvr
        rvals = [0.129/0.134, 0.075/0.134]
        mw_av = 0.134
        ho_av = 0.20
        for data in [[27.422,0.321,28.791,0.146],
                     [23.131, 0.011, 24.051, 0.004]]:

            dm = 5 * np.log10(34.0e6) - 5.0
            dmerr = 0.2

            yval = data[2] - dm - mw_av * rvals[1] - ho_av * rvals[1]
            yerr = np.sqrt(dmerr**2 + data[3]**2)

            xval = data[2] - data[0] - (rvals[1]-rvals[0]) * mw_av -\
                (rvals[1]-rvals[0]) * ho_av
            xerr = np.sqrt(data[1]**2 + data[3]**2)

            ax.errorbar(xval, yval, yerr=yerr, xerr=xerr, capsize=0.5*self.figsize,
                linewidth=0.6*self.figsize, color=black,
                marker='*', ms=4*self.figsize,
                markeredgecolor=black, markeredgewidth=0.4*self.figsize,
                capthick=0.8*self.figsize, zorder=10)

        self.close_plot('cmd-'+filt1+'-'+filt2+'.eps')
...
def main():
    ...
    photfile = sed.fit.dirs['input']+'2020fqv.dat'
    sed.load_sed('blackbody', photfile)

    #sed.plot_corner(['bpass'])
    print('Making HR figure...')
    sed.cmd('F350LP', 'F814W', progenitors='progenitors.dat',
        add_progenitors=True,add_yoon=False,add_lbvs=False,add_data=True,
