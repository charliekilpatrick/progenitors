        ...
        if add_data:
            if limits:
                if self.fit.model_type=='blackbody':
                    ...
                    photfile = self.fit.dirs['input']+'2021yja.dat'
                    ...
                    print(ylim,xlim)
                    mask = np.array(['wfpc2' not in inf.lower() for inf in inst_filt])
                    for ii,temp in enumerate(tem_space):
                        for jj,lum in enumerate(lum_space):
                            ...
                            if any([mmag>nmag for mmag,nmag in zip(mag,model)]):
                                grid[ii,jj]=1
                            if any([mmag>nmag+0.42 for mmag,nmag in zip(mag,model)]):
                                grid_lo[ii,jj]=1
                            if any([mmag>nmag-0.42 for mmag,nmag in zip(mag,model)]):
                                grid_hi[ii,jj]=1
                            if any([mmag>nmag for mmag,nmag in zip(mag[mask],model[mask])]):
                                grid_nowfpc2[ii,jj]=1
                    ...
                    ax.contour(tem_space,lum_space,np.transpose(grid_lo),
                        linewidths=6, levels=levels, zorder=5, colors=('darkgoldenrod'))
                    ...
                    for ii,temp in enumerate(tem_space):
                        for jj,lum in enumerate(lum_space):
                            ...
                            if any([mmag>nmag for mmag,nmag in zip(mag,model)]):
                                grid[ii,jj]=1
                            if any([mmag>nmag+0.45 for mmag,nmag in zip(mag,model)]):
                                grid_lo[ii,jj]=1
                            if any([mmag>nmag-0.45 for mmag,nmag in zip(mag,model)]):
                                grid_hi[ii,jj]=1
                            if any([mmag>nmag for mmag,nmag in zip(mag[mask],model[mask])]):
                    ...
                    ax.contour(tem_space,lum_space,np.transpose(grid),
                        linewidths=6, levels=levels, zorder=5,
                        colors=('red'))

                    dx=-0.1
                    ax.text(4.63, 5.15, '$E(B-V)_{\\rm host}$=0 mag', fontsize=3.6*self.figsize,
                        color='red', rotation=0, va='center', ha='center',
                        weight='bold',
                        bbox=dict(facecolor='white', edgecolor='red'),
                        zorder=15)
                    ax.text(3.90, 4.16, '$E(B-V)_{\\rm host}$=0.085 mag', fontsize=3.6*self.figsize,
                        color='blue', rotation=0, va='center', ha='center',
                        weight='bold',
                        bbox=dict(facecolor='white', edgecolor='blue'),
                        zorder=15)
                    ax.text(3.95, 4.75, '$D_{L}$=28.8 Mpc', fontsize=3.6*self.figsize,
                        color='darkgoldenrod', rotation=0, va='center', ha='center',
                        weight='bold',
                        bbox=dict(facecolor='white', edgecolor='darkgoldenrod'),
                        zorder=15)
                    #ax.text(4.62, 5.45, '$E(B-V)$=0.085 mag', fontsize=2.5*self.figsize,
                    #    color='darkgoldenrod', rotation=-46, va='center', ha='center')


                    ax.text(3.65, 5.45, 'Ruled Out', fontsize=3.9*self.figsize)

                    radius = [30, 60, 100, 300, 600, 1000]
                    rlums = np.zeros((len(tem_space), len(radius)))
                    for i,r in enumerate(radius):
                        for jj,temp in enumerate(tem_space):
                            rlums[jj,i]=np.log10(8.978e-16*(10**temp)**4*r**2)

                    for i,r in enumerate(radius):
                        ax.plot(tem_space, rlums[:,i], color='lightgray',
                            zorder=20)
...
def main():
    ...
    photfile = sed.fit.dirs['input']+'2021yja.dat'
