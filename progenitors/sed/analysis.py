"""
SED fitter: infer stellar parameters from photometry.

Compares input photometry to MIST tracks, blackbodies, Pickles SEDs, BPASS,
and red supergiant models (Kilpatrick & Foley 2018). Supports MCMC (emcee)
...
# Local alias for parameter lists by model type (copy so we can override 'rsg' for notau)
model_fit_params = dict(MODEL_FIT_PARAMS)
...
class sed_fitter(object):
    def __init__(self, verbose=True, interpolate=True):
        ...
        self.dirs = {
            'bandpass': os.path.join(self._sed_root, 'data', 'bandpass') + os.sep,
            'extinction': os.path.join(self._sed_root, 'data', 'extinction') + os.sep,
            'input': os.path.join(self._sed_root, 'data', 'input') + os.sep,
            'mist': os.path.join(self._sed_root, 'data', 'mist') + os.sep,
            'bpass': os.path.join(self._sed_root, 'data', 'bpass') + os.sep,
            'figures': os.path.join(self._sed_root, 'figures') + os.sep,
            'backends': os.path.join(self._sed_root, 'data', 'backends') + os.sep,
            'data': os.path.join(self._sed_root, 'data') + os.sep,
            'bcs': os.path.join(self._sed_root, 'data', 'bcs') + os.sep,
        }
        ...
        # Default wavelength binset in angstroms for synphot
    ...
    def add_options(self, parser=None, usage=None):
        ...
        parser.add_argument('--sampler', default='emcee', type=str,
            ...
            help='Sampler to use: emcee (MCMC) or dynesty (nested sampling).')
        parser.add_argument('--nlive', type=int, default=500,
            help='Number of live points for dynesty nested sampling.')
        parser.add_argument('--dynesty-bound', default='multi', type=str,
            choices=['single', 'multi', 'balls', 'cubes'],
            help='Dynesty bound method (single/multi/balls/cubes).')
    ...
    def set_model_type(self, model_type, extinction=False, notau=False):
        ...
        _params = dict(model_fit_params)
        if "rsg" in model_type and notau:
            _params["rsg"] = ["luminosity", "temperature"]
        self.model_fit_params = _params[model_type]
    ...
    # outputs a synphot spectrum with relative extinction versus wavelength
    ...
    # Loads a file from the pickles stellar SED atlas (uses synphot_compat).
    def import_pickles_file(self, filename):
        if '.fits' in filename: filename=filename.replace('.fits','')
        root = S.get_spectral_data_root()
        if not root:
            return None
        fullfile = os.path.join(root, 'grid', 'pickles', 'dat_uvk', filename+'.fits')
    ...
    def load_pickles(self, phottable, **kwargs):
        ...
        if not self.interpolate:
            ...
            for j,logT in enumerate(logTval):
                ...
                for i,L in enumerate(Lval):
                    scaled_spec = sp * 10**L
                    mag = self.compute_synphot_mag(scaled_spec, bandpasses)
    ...
    def blackbody(self, temp, wave=None):
        ...
        bb = S.ArraySpectrum(wave, flux, waveunits='angstrom', fluxunits='flam')
    ...
    def compute_blackbody_mag(self, inst_filt, *args):
        ...
        bandpasses = self.get_bandpasses(inst_filt)
        mags = self.compute_synphot_mag(bb, bandpasses)
    ...
    def run_emcee(self, phottable, sigma=1.0, nsteps=5000, nwalkers=100,
        ...
        self._report_sampler_results(sample, prob, blob, ndim, inst_filt,
            mag, magerr, mjd)

    def _report_sampler_results(self, sample, prob, blob, ndim, inst_filt,
        mag, magerr, mjd):
        """
        Report best-fit parameters, radius, dust, and model mags from sampler output.

        Parameters
        ----------
        sample : ndarray
            Shape (n_steps, ndim); parameter samples.
        prob : ndarray
            Log-probability for each sample.
        blob : ndarray
            Shape (n_steps, n_blobs); e.g. Av, Rv per sample.
        ndim : int
            Number of model parameters.
        inst_filt : list of str
            Instrument/filter names.
        mag, magerr : ndarray
            Magnitudes and errors.
        mjd : ndarray
            MJD for each measurement.
        """
        model_type = self.model_type
        params = []
        blobs = []
    ...
    def run_dynesty(self, phottable, nlive=500, bound='multi', guess_type='params'):
        """
        Run nested sampling with dynesty.

        Uses the same likelihood as run_emcee. When extinction is modeled,
        Av and Rv are included as sampled parameters.

        Parameters
        ----------
        phottable : Table
            Photometry table (used for get_fit_parameters).
        nlive : int, optional
            Number of live points. Default 500.
        bound : {'single', 'multi', 'balls', 'cubes'}, optional
            Dynesty bound method. Default 'multi'.
        guess_type : str, optional
            Passed to get_guess (unused for dynesty). Default 'params'.
        ...
        """
        if not _HAS_DYNESTY:
            raise ImportError('dynesty is required for run_dynesty. Install with: pip install dynesty')
        model_type = self.model_type
        self.make_banner(f'Starting nested sampling (dynesty) for {model_type}')

        mjd, inst_filt, mag, magerr = self.get_fit_parameters(phottable)
        nparams = len(self.model_fit_params)
        include_ext = (self.extinction_model and self.model_fit_blobs)
        if include_ext:
            param_names = list(self.model_fit_params) + ['Av', 'Rv']
            bounds_list = [self.bounds.get(p, [0.0, 1.0]) for p in self.model_fit_params]
            bounds_list.append(self.bounds['Av'])
            bounds_list.append(self.bounds['Rv'])
        else:
            param_names = list(self.model_fit_params)
            bounds_list = [self.bounds.get(p, [0.0, 1.0]) for p in self.model_fit_params]

        def _cap(b):
            lo, hi = (b[0], b[1]) if len(b) >= 2 else (0.0, 1.0)
            hi = hi if np.isfinite(hi) else 10.0
            return (float(lo), float(hi))

        ndim = len(param_names)
        bounds_arr = np.array([_cap(b) for b in bounds_list])

        def prior_transform(u):
            return bounds_arr[:, 0] + u * (bounds_arr[:, 1] - bounds_arr[:, 0])

        def logl(theta):
            if include_ext:
                theta_mod = theta[:nparams]
                ext = (float(theta[-2]), float(theta[-1]))
            else:
                theta_mod = theta
                ext = None
            ll, _, _ = self.log_likelihood(theta_mod, inst_filt, mag, magerr, extinction=ext)
            return ll

        sampler = dynesty.NestedSampler(
            logl, prior_transform, ndim,
            bound=bound, nlive=nlive,
        )
        sampler.run_nested(print_progress=self.verbose)
        res = sampler.results

        samples = res.samples
        logl_vals = res.logl
        weights = np.exp(res.logwt - res.logz[-1])
        inds = np.random.choice(len(weights), size=min(2000, len(weights)), p=weights, replace=True)
        sample = samples[inds]
        prob = logl_vals[inds]

        if include_ext:
            sample = sample[:, :nparams]
            blob = samples[inds][:, nparams:]
        else:
            nblob = len(self.model_fit_blobs)
            if nblob and self.host_ext:
                blob = np.tile(self.host_ext, (len(sample), 1))
            else:
                blob = np.zeros((len(sample), max(1, nblob)))

        self._report_sampler_results(sample, prob, blob, nparams, inst_filt,
            mag, magerr, mjd)
...
def main():
    ...
    for model in run_models:
        ...
        if opt.sampler == 'dynesty':
            sed.run_dynesty(sed.phottable, nlive=opt.nlive, bound=opt.dynesty_bound)
        else:
