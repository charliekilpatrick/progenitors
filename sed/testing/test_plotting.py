...
class sed_plot(object):
    ...
    def plot_sed(self, models, fluxspace=True, extinction=False, logwave=False):
        ...
        for i,model in enumerate(models):
            ...
            print('Using extinction values:',ext)
            print('Milky Way E(B-V):',self.fit.mw_ebv)
            ...
            if i==0:
                for w,f,fe,wi in zip(wave, flux, fluxerr, width):
                    if fe==0:
                        uplim=np.array([1])
                        fe=0.5*f
                    else:
                        uplim=np.array([0])
                    ax.errorbar(w, f, yerr=fe, xerr=wi, uplims=uplim,
...
def main():
    ...
    sed.load_sed('rsg', photfile)
    sed.plot_sed(['rsg'], logwave=True)
